...
## chr(33) == '!' represents A of clubs,
## chr(34) == '"' represents 2 of clubs, and so on in order until
## chr(84) == 'T' represents K of spades.
...
## input files (or stdin if none specified) into the variable $o
while( <> )
{
...
##   to get the range 0..25, add 65 to get back the range 65..90,
##   and determine the character with that ASCII value
##   ( chr( (ord($&)-13 + $f*&e)%26 + 65 ) ), which is between 'A' and 'Z'.
##   Replace the original character with this new one.
$o =~ s/./chr( (ord($&) - 13 + $f * &e)%26 + 65 )/eg ;

## If we're decrypting, remove trailing X's from the newly found plaintext
# >> BUG HERE!!  If the original plaintext actually ends with any number
#                of 'x' or 'X', they are lost.
$o =~ s/X*$// if $d;
